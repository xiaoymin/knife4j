...
:::danger 友情提示
该文档页介绍已作废,请移步新的[文档介绍](/docs/middleware-sources/desktop-introduction)
:::
...
`disk.properties`配置文件(该配置和[Knife4jAggregation聚合组件](knife4jAggregation)中声明的disk模式的route节点配置完全一样)：
```properties
...
|--------cloud.properties
```
`cloud.properties`配置文件(该配置和[Knife4jAggregation聚合组件](knife4jAggregation)中声明的cloud模式的route节点配置完全一样)：
...
|--------eureka.properties
```
`eureka.properties`配置文件(该配置和[Knife4jAggregation聚合组件](knife4jAggregation)中声明的cloud模式的route节点配置完全一样)：
...
|--------nacos.properties
```
`nacos.properties`配置文件(该配置和[Knife4jAggregation聚合组件](knife4jAggregation)中声明的nacos模式的route节点配置完全一样)：
