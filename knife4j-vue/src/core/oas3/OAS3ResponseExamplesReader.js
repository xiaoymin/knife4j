 ...
 * @param {*} source 请求原始对象
 */
function Knife4jOAS3ResponseExamplesReader(source) {
    this.source = source;
 ...
 */
Knife4jOAS3ResponseExamplesReader.prototype.init = function () {
    // 判断当前是否包含example
 ...
 * @param {*} example 示例值
 */
Knife4jOAS3ResponseExamplesReader.prototype.oneExample = function (example) {
 ...
 */
Knife4jOAS3ResponseExamplesReader.prototype.multipleExample = function (moreExample) {
    if (KUtils.checkUndefined(moreExample)) {
        for (let key in moreExample) {
            ...
            if (KUtils.checkUndefined(singleExample)) {
                ...
                if (KUtils.checkUndefined(summary)) {
                    ...
                    let example = KUtils.propValue('value', singleExample, null);
        ...
        if (KUtils.checkUndefined(this.responseTextArray)) {
            //取第一个
            let responseOne = this.responseTextArray[0];
            this.responseText = responseOne.example;
            this.responseValue = responseOne.example;
        }
    }
}

export default Knife4jOAS3ResponseExamplesReader;
