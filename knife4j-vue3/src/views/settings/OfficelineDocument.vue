    ...
    <div class="markdown-row">
      ...
      <a-row class="knife4j-download-button">
        ...
        <a-button type="default" @click="triggerDownloadHtml">
          <FileTextOutlined /><span >{{ $t('offline.download.html') }}</span >
        </a-button>
        <a-button type="default" @click="triggerDownloadWord">
          <FileWordOutlined /><span>{{ $t('offline.download.word') }}</span>
        </a-button>
        <a-button type="default" @click="triggerDownloadOpenAPI">
          <FileTextOutlined /><span>OpenAPI</span>
        </a-button>
...
import { FileMarkdownOutlined, FileTextOutlined, FileWordOutlined } from '@ant-design/icons-vue'
...
export default {
  ...
  methods: {
    ...
    triggerDownloadPDF() {
      // var message='该功能尚未实现...'
      const msg = this.getCurrentI18nInstance().message.offline.imple
      message.info(msg);
    },
    triggerDownloadWord() {
      const that = this
      // 正在下载word文件中,请稍后...
      const downloadMessage = this.getCurrentI18nInstance().message.offline.word
      that.kloading.show({
      ...
      const markdownKey = this.data.instance.id + 'markdownFiles'
      localStore.getItem(markdownKey).then(mdfileMap => {
    ...
    triggerDownloadMarkdown() {
      ...
      var that = this;
      // 正在下载Markdown文件中,请稍后...
      const downloadMessage = this.getCurrentI18nInstance().message.offline.markdown
      that.kloading.show({
        text: downloadMessage
      });
      this.deepTags();
      ...
      localStore.getItem(markdownKey).then(mdfileMap => {
        // console.log(mdfileMap)
        const markdownFiles = that.data.instance.markdownFiles
        if (KUtils.checkUndefined(mdfileMap)) {
          if (KUtils.arrNotEmpty(markdownFiles)) {
            markdownFiles.forEach(mdgrp => {
              // 判断是否children
